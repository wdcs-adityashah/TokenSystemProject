    ...
    socket.current.on("table-reservation-updated", (data: Reservation) => {
      ...
      setReservations((prev) => {
        const index = prev.findIndex(
          (res) => res.tableNumber === data.tableNumber
        );
        if (index > -1) {
          const updatedReservations = [...prev];
          updatedReservations[index] = {
  ...
  const handleReserveTable = async (
    ...
    const response = await fetch(
      "http://localhost:2000/api/tables/reserve-table",
      {
        ...
        body: JSON.stringify({ tableNumber, isReserved, isProcessed,userId }),
      }
    );
              ...
              filteredReservations.map((reservation) => (
                ...
                >
                  ...
                  <td className="border px-4 py-2">
                    <button
                      className={`bg-${
                        reservation.isReserved ? "red" : "green"
                      }-500 text-white px-4 py-2 rounded transition duration-300 hover:opacity-80`}
                      onClick={() =>
                        handleReserveTable(
                          reservation.tableNumber,
        ...
        {availableTables.length > 0 ? (
          availableTables.map((tableNumber) => (
            ...
            >
              ...
              <button
                className="mt-2 bg-green-500 text-white px-4 py-2 rounded transition duration-300 hover:opacity-80"
                onClick={() => handleReserveTable(tableNumber, true, true)}
              >
                Reserve Now
              </button>
            </div>
          ...
          <p className="col-span-full text-2xl text-center text-red-500">
            No tables available.
          </p>
        )}
      </div>
